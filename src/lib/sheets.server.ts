// Google Sheets order sync. Requires:
//   - GOOGLE_SHEETS_API_KEY or GOOGLE_SHEETS_ACCESS_TOKEN
//   - GOOGLE_SHEETS_SPREADSHEET_ID secret set to the target sheet
// If either is missing the sync becomes a no-op so order placement never fails.

export async function appendOrderToSheet(orderId: string) {
    ...
    const { data: order, error } = await supabaseAdmin
      .from("orders")
      .select("order_number, status, total, email, phone, shipping_address, created_at, order_items(name, quantity, price)")
      .eq("id", orderId)
      .maybeSingle();
