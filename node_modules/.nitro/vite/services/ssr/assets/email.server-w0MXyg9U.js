...
async function sendNewDeviceLoginAlert(email, details) {
  await sendEmail({
    to: email,
...
async function sendPasswordChangedAlert(email, details) {
  await sendEmail({
    to: email,
    subject: "Your IRAYA password was changed",
    html: `
      <h2>Password Changed</h2>
      <p>Your account password was successfully changed at ${(/* @__PURE__ */ new Date()).toLocaleString()} from IP address ${details.ip}.</p>
...
async function sendEmailChangeAttemptAlert(oldEmail, details) {
  await sendEmail({
    to: oldEmail,
...
async function sendNewDeliveryAddressAlert(email, details) {
  await sendEmail({
    to: email,
...
async function sendSuspiciousLoginBlockedAlert(email, details) {
  await sendEmail({
    to: email,
...
async function sendAccountLockedAlert(email, details) {
  await sendEmail({
    to: email,
...
async function sendOrderPlacedConfirmation(email, details) {
  await sendEmail({
    to: email,
...
export {
  ...
  sendEmail,
  sendEmailChangeAttemptAlert,
  sendNewDeliveryAddressAlert,
